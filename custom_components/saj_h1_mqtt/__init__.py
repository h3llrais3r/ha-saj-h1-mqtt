...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    scan_interval_inverter_info: int | str | None = None
    if entry.data[CONF_ENABLE_INVERTER_INFO]:
        scan_interval_inverter_info = (
            entry.data[CONF_SCAN_INTERVAL_INVERTER_INFO] or STARTUP
        )

    scan_interval_battery_info: int | str | None = None
    if entry.data[CONF_ENABLE_BATTERY_INFO]:
        scan_interval_battery_info = (
            entry.data[CONF_SCAN_INTERVAL_BATTERY_INFO] or STARTUP
        )

    scan_interval_battery_controller: int | str | None = None
    if entry.data[CONF_ENABLE_BATTERY_CONTROLLER]:
        scan_interval_battery_controller = (
            entry.data[CONF_SCAN_INTERVAL_BATTERY_CONTROLLER] or STARTUP
        )

    scan_interval_config: int | str | None = None
    if entry.data[CONF_ENABLE_CONFIG]:
        scan_interval_config = entry.data[CONF_SCAN_INTERVAL_CONFIG] or STARTUP

    debug_mqtt: bool = entry.data[CONF_ENABLE_MQTT_DEBUG]
    ...
    hass.data[DOMAIN][entry.entry_id][DATA_SAJMQTT] = saj_mqtt
    ...
    if scan_interval_config:
        ...
        hass.data[DOMAIN][entry.entry_id][DATA_COORDINATOR_CONFIG] = coordinator_config
